...
function mapToolUseToActions(
  ...
  for (const { tool, input } of toolUses) {
    switch (tool) {
        ...
        if (input.title && input.amount)
          mapped.push({
            ...
            expense: {
              ...
              paid_by: (input.paid_by as 'aba' | 'ima' | 'kid1' | 'kid2' | 'kid3') || 'aba',
      ...
      case 'add_task':
        mapped.push({
          ...
          task: {
            ...
            group: (input.group as 'pre_trip' | 'during_trip' | 'post_trip') || 'pre_trip',
            assigned_to: (input.assigned_to as FamilyMemberId[]) || ['aba'],
      ...
      case 'complete_task':
        if (input.task_title)
          mapped.push({ type: 'COMPLETE_TASK', taskTitle: String(input.task_title) })
        break
      case 'add_note':
        mapped.push({
          type: 'ADD_NOTE',
      ...
      case 'toggle_packing_item':
        if (input.item_name)
          mapped.push({ type: 'TOGGLE_PACKING_ITEM', itemName: String(input.item_name) })
        break
        ...
        if (input.day_id && input.title)
          mapped.push({
            ...
            stop: {
              ...
              start_time: input.start_time ? String(input.start_time) : undefined,
            },
          })
        break
...
const TRIP = {
  ...
  duration: '21 יום',
  route:
    "תל אביב → ניוארק → בוזמן → ילוסטון → ג'קסון → ברייס קניון → זאיון → לאס וגאס → יוסמיטי → סן פרנסיסקו → תל אביב",
  flights: {
    outbound: 'United UA85, TLV → EWR (ניוארק), 10 בספטמבר 2026, 11:00–15:50. טיסה ישירה',
    domestic: 'United UA297, EWR → BZN, 11 בספטמבר 2026, 07:45–10:27, מושבים 36A-36E',
    return: 'SFO → MUC → TLV, 30 בספטמבר 2026, אישור HQ51BY',
  },
...
const rules: MatchRule[] = [
    ...
    response: () =>
      wrap(
        `פרטי הטיסות שלכם:\n\n` +
          ...
          `   ↳ טיסה ישירה לניוארק, נחיתה 15:50. לינה במלון ליד נמל התעופה\n\n` +
          `🛫 **פנימית (11/9):** ${TRIP.flights.domestic}\n` +
          `   ↳ מניוארק לבוזמן, משם איסוף הקרוואן. יציאה מוקדמת — קומו ב-05:00\n\n` +
  ...
  {
    keywords: ['הכי הרבה זמן נסיעה', 'יום נסיעה ארוך', 'longest drive', 'הארוך ביותר'],
    response: () =>
      wrap(
        `📅 **יום 8 (17/9) — יום הנסיעה הארוך ביותר**\n\n` +
          `🚗 נסיעה של כ-**7 שעות** מג׳קסון, ויומינג → פרובו/נפי, יוטה.\n\n` +
          `💡 כדאי לצאת מוקדם, לתכנן הפסקות כל שעתיים, ולהכין בידור לדרך.`,
      ),
  },
  {
    keywords: ['נסיעה', 'כמה זמן', 'כמה שעות', 'כמה רחוק', 'מרחק', 'drive'],
    response: () =>
      wrap(
  ...
  {
    keywords: ['ניוארק', 'newark', 'ewr', 'בוזמן', 'bozeman', 'מונטנה', 'montana'],
    response: () =>
      wrap(
        `**ניוארק** — יום 1 (10/9): נחיתה בלבד!\n` +
          `נחיתה ב-15:50 (UA85 מתל אביב), ישר למלון ליד השדה. אל תנסו לקפוץ למנהטן — הטיסה לבוזמן ב-07:45 בבוקר.\n\n` +
          `💡 קומו ב-05:00 לכל המאוחר. בחרו מלון עם שאטל לטרמינל, זה חוסך המון בבוקר.\n\n` +
          `---\n\n` +
          `**בוזמן** — יום 2 (11/9): הכי חשוב!\n` +
          `• **07:45–10:27** — טיסה פנימית EWR → BZN (UA297)\n` +
    ...
    response: () =>
      wrap(
        `גובה — הדבר שכולם שוכחים לתכנן בשבילו!\n\n` +
          ...
          `• ג'קסון, WY: **1,900 מ'** (הגוף מתחיל להסתגל כבר שם)\n\n` +
          `**תסמיני גובה:**\n` +
    ...
    response: () =>
      wrap(
        `כיסוי סלולרי ו-SIM בארה"ב:\n\n` +
          ...
          `**כיסוי לאורך המסלול:**\n` +
          `• ניוארק / בוזמן / ג'קסון / וגאס / SF: ✅ מלא\n` +
