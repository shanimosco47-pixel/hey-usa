...
const components: Components = {
  ...
  ul: ({ children }) => <ul className="list-disc pe-4 mb-1.5 space-y-0.5">{children}</ul>,
  ol: ({ children }) => <ol className="list-decimal pe-4 mb-1.5 space-y-0.5">{children}</ol>,
  ...
  blockquote: ({ children }) => (
    <blockquote className="border-e-2 border-ios-blue/30 pe-3 my-1.5 text-apple-secondary">
      {children}
    </blockquote>
  ),
  table: ({ children }) => (
  ...
  th: ({ children }) => (
    <th className="text-start border-b border-black/10 pb-1 pe-2 font-semibold">{children}</th>
  ),
  td: ({ children }) => (
    <td className="border-b border-black/5 py-1 pe-2">{children}</td>
  ),
