...
`)},{role:"assistant",content:"תודה, אני מעודכן. במה אפשר לעזור?"})}const S=await fetch(`${e}/functions/v1/moti-chat`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${r}`,apikey:r},body:JSON.stringify({messages:p,appContext:t||""})});if(S.ok){const i=await S.json();if(i?.text!==void 0){const l=i.text;b.push({role:"assistant",content:l});const f=De(i.actions||[]),w=a.length>0?a:f;return{text:l,actions:w}}}console.warn("AI request failed with status:",S.status)}catch(p){console.warn("AI request failed, falling back to keywords:",p)}const h=Oe(s);return b.push({role:"assistant",content:h}),{text:h,actions:[]}}function je(s){const t=[];for(const a of s)switch(a.type){case"UPDATE_BUDGET_CATEGORY":{const e=M[a.category]||a.category;t.push(`בוצע! עדכנתי את תקציב **${e}** ל-**₪${a.amount.toLocaleString()}**. ✅
...
• נהיגה בצד ימין, לא שמאל. כן, אני צריך לומר את זה.`)},{keywords:["דיסנילנד","דיסני","disney"],response:()=>u(`דיסנילנד! הילדים ישתגעו (וההורים יישברו כלכלית, אבל שווה).
...
• תקציב אוכל בפארק: תוסיפו 30% על מה שחשבתם. רציני.`)},{keywords:["גרנד קניון","grand canyon","קניון"],response:()=>u(`גרנד קניון — **15 בספטמבר**.
...
• שקיעה מ-Mather Point = הרגע הכי שווה בטיול`)},{keywords:["יוסמיטי","yosemite"],response:()=>u(`יוסמיטי — **18-20 בספטמבר** (3 ימים!).
