...
Deno.serve(async (req) => {
  ...
  if (accountsError) {
    ...
    return corsResponse(
      JSON.stringify({ error: "Failed to fetch email accounts" }),
      { status: 500 },
    );
  }
  ...
  for (const account of accounts) {
    ...
    for (const ref of messageRefs) {
      try {
        ...
        // Dedup check
        const alreadyImported = await isAlreadyImported(
          supabase,
          message.id,
          bodyText,
          subject,
    ...
    if (updateError) {
      console.error(`[email-scan] Failed to update last_scan_at for ${account.email}:`, updateError.message);
    }
  }
