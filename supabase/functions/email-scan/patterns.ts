...
function isKnownSender(fromEmail: string): boolean {
  ...
  return KNOWN_SENDER_DOMAINS.some(
...
export function buildSearchQuery(lastScanAt: string | null): string {
  ...
  // Subject keyword OR group (English only for Gmail search)
  const subjectParts = SUBJECT_KEYWORDS_EN.map(
    (kw) => `subject:"${kw}"`,
  );
...
export function extractBookingRefs(text: string): string[] {
  ...
  const labelRegex =
    /(?:order|confirmation|booking|ref(?:erence)?)\s*[:#\-\s]+([A-Z0-9]{4,})/gi;
